...
async function searchMovies(userGenres, userLangs, userPopularity) {
    const response = await fetch(`${URL}/discover/movie?api_key=${API_KEY}&with_genres=${userGenres}&with_original_language=${userLangs}&vote_average.${getPopularity()}`);
...
const genreNameMap = {
  ...
  function getGenresName(userGenres) {
    return userGenres.map(id => genreNameMap[id] || "tidak diketahui").join(", ");
  }
